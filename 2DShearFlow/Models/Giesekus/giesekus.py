...
T = -p*I  + 2 * nu * sym(grad(u)) + mu*(Bp - I)
BEq = mu*Bp*(Bp - I) + nu1*(Constant(1.0/dt)*(Bp-Bp0) + dot(grad(Bp),u) - L*Bp - Bp*L.T)
#NEq = (c1*I + c2*Bp)*np - L*np
F = Constant(1.0/dt)*inner(u-u0, v)*dx + inner(T, grad(v))*dx - q*div(u)*dx + inner(grad(u)*u, v)*dx + inner(BEq, Bpt)*dx
